...
@section('title')
	Buku Induk | SD Negeri Sumbergandu
@stop
					...
					<div class="panel">
						...
						<div class="panel-body">
							<div class="row">
								...
								<div class="col-md-3">
									<form class="" method="GET" action="/data-buku-induk/SDN-Sumbergandu">
								...
								<table class="table table-striped">
									...
									<tbody>
										...
										<tr>
											...
											<td><a href="/buku-induk/{{$r->id}}/SDN-Sumbergandu">{{ $r->nis }}</a></td>
											<td><a href="/buku-induk/{{$r->id}}/SDN-Sumbergandu">{{ $r->nama_lengkap }}</a></td>
											<td>{{ $r->jenis_kelamin }}</td>
											<td>{{ $r->ttl }}</td>
											<td>{{ $r->alamat }}</td>
											@if(auth()->user()->role == 'Admin')
											<td>
												<a href="/edit-buku-induk/{{$r->id}}/SDN-Sumbergandu" class="btn btn-warning btn-sm"><i class="lnr lnr-pencil"></i></a>
	...
	<script type="text/javascript">
		$('.delete').click(function(){
			...
			swal({
				...
				.then((willDelete) => {
				  if (willDelete) {
				  	window.location = "/destroy-buku-induk/"+bi_id+"/SDN-Sumbergandu";
				  }
				});
		});
	</script>
